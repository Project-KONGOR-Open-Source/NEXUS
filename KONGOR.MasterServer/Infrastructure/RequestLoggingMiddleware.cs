...
public class RequestLoggingMiddleware(RequestDelegate next, ILogger<RequestLoggingMiddleware> logger)
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        if (context.Request.ContentLength > 0 && (context.Request.ContentType?.Contains("form") == true ||
                                                  context.Request.ContentType?.Contains("json") == true))
        {
            using StreamReader reader = new(context.Request.Body, Encoding.UTF8, false, leaveOpen: true);
