namespace KONGOR.MasterServer.Models.RequestResponse.Stats;

/// <summary>
///     Response for show_stats with table="campaign_casual" (casual ranked matchmaking statistics with cam_cs_ prefix).
/// </summary>
public class CampaignCasualStatisticsResponse(Account account, AccountStatistics statistics, AggregateStatistics aggregates)
{
    /// <summary>
    ///     The per-hero statistics totals for this game mode, aggregated across every hero the account has played.
    /// </summary>
    private HeroStats Totals { get; } = statistics.HeroStatistics.AggregateTotals();

    /// <summary>
    ///     The account's up-to-five most-played heroes for this game mode, ordered by matches played descending.
    /// </summary>
    private IReadOnlyList<FavouriteHero> FavouriteHeroes { get; } = StatisticsResponseHelper.GetFavouriteHeroes(statistics);

    /// <summary>
    ///     The account's super ID (main account ID).
    /// </summary>
    [PHPProperty("super_id")]
    public string SuperID { get; init; } = account.User.Accounts.Single(account => account.IsMain).ID.ToString();

    /// <summary>
    ///     The account name with the clan tag (if applicable).
    /// </summary>
    [PHPProperty("nickname")]
    public string Nickname { get; init; } = account.NameWithClanTag;

    /// <summary>
    ///     The account standing (moderation status).
    /// </summary>
    [PHPProperty("standing")]
    public string Standing { get; init; } = "3"; // TODO: Implement Account Standing

    /// <summary>
    ///     The account type.
    /// </summary>
    [PHPProperty("account_type")]
    public string AccountTypeValue { get; init; } = ((int) account.Type).ToString();

    /// <summary>
    ///     The unique account identifier.
    /// </summary>
    [PHPProperty("account_id")]
    public string AccountID { get; init; } = account.ID.ToString();

    /// <summary>
    ///     The account level.
    /// </summary>
    [PHPProperty("level")]
    public string Level { get; init; } = account.User.TotalLevel.ToString();

    /// <summary>
    ///     The current experience points towards the next level.
    /// </summary>
    [PHPProperty("level_exp")]
    public int LevelExperience { get; init; } = account.User.TotalExperience;

    /// <summary>
    ///     The date of last activity on the account.
    /// </summary>
    [PHPProperty("last_activity")]
    public string LastActivity { get; init; } = account.TimestampLastActive.ToString("MM/dd/yyyy");

    /// <summary>
    ///     The date the account was created.
    /// </summary>
    [PHPProperty("create_date")]
    public string CreateDate { get; init; } = account.TimestampCreated.ToString("MM/dd/yyyy");

    /// <summary>
    ///     The current season number.
    /// </summary>
    [PHPProperty("season")]
    public string Season { get; init; } = SeasonInformation.CurrentSeasonIndex.ToString();

    /// <summary>
    ///     The clan name, or an empty string if the account is not in a clan.
    /// </summary>
    [PHPProperty("name")]
    public string ClanName { get; init; } = account.Clan?.Name ?? string.Empty;

    /// <summary>
    ///     The clan rank (tier) name.
    /// </summary>
    [PHPProperty("rank")]
    public string ClanRank { get; init; } = account.ClanTierName;

    # region Campaign Casual Match Statistics (cam_cs_ Prefix)

    /// <summary>
    ///     Total campaign casual matches played.
    /// </summary>
    [PHPProperty("cam_cs_games_played")]
    public string GamesPlayed { get; init; } = statistics.MatchesPlayed.ToString();

    /// <summary>
    ///     Total campaign casual matches won.
    /// </summary>
    [PHPProperty("cam_cs_wins")]
    public string Wins { get; init; } = statistics.MatchesWon.ToString();

    /// <summary>
    ///     Total campaign casual matches lost.
    /// </summary>
    [PHPProperty("cam_cs_losses")]
    public string Losses { get; init; } = statistics.MatchesLost.ToString();

    /// <summary>
    ///     Total campaign casual matches conceded.
    /// </summary>
    [PHPProperty("cam_cs_concedes")]
    public string Concedes { get; init; } = statistics.MatchesConceded.ToString();

    /// <summary>
    ///     Total times voted to concede in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_concedevotes")]
    public string ConcedeVotes => Totals.ConcedeVotes.ToString();

    /// <summary>
    ///     Total buybacks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_buybacks")]
    public string Buybacks => Totals.Buybacks.ToString();

    /// <summary>
    ///     Total disconnections in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_discos")]
    public string Disconnections { get; init; } = statistics.MatchesDisconnected.ToString();

    /// <summary>
    ///     Total times kicked from campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_kicked")]
    public string Kicked { get; init; } = statistics.MatchesKicked.ToString();

    /// <summary>
    ///     Solo queue matchmaking rating (MMR) for campaign casual mode.
    /// </summary>
    [PHPProperty("cam_cs_amm_solo_rating")]
    public string SoloRating { get; init; } = statistics.SkillRating.ToString("F2", CultureInfo.InvariantCulture);

    /// <summary>
    ///     Total solo queue campaign casual matches counted towards rating.
    /// </summary>
    [PHPProperty("cam_cs_amm_solo_count")]
    public string SoloCount { get; init; } = statistics.MatchesPlayed.ToString();

    /// <summary>
    ///     Solo queue campaign casual rating confidence (0.00 to 1.00).
    /// </summary>
    [PHPProperty("cam_cs_amm_solo_conf")]
    public string SoloConfidence { get; init; } = "0.00"; // TODO: Implement Data Tracking

    /// <summary>
    ///     Number of provisional (placement) matches played in solo queue campaign casual.
    /// </summary>
    [PHPProperty("cam_cs_amm_solo_prov")]
    public string SoloProvisional { get; init; } = "0"; // TODO: Implement Data Tracking

    /// <summary>
    ///     Solo queue campaign casual placement set identifier.
    /// </summary>
    [PHPProperty("cam_cs_amm_solo_pset")]
    public string SoloPlacementSet { get; init; } = "0"; // TODO: Implement Data Tracking

    /// <summary>
    ///     Team/group queue matchmaking rating (MMR) for campaign casual mode.
    /// </summary>
    [PHPProperty("cam_cs_amm_team_rating")]
    public string TeamRating { get; init; } = statistics.SkillRating.ToString("F2", CultureInfo.InvariantCulture);

    /// <summary>
    ///     Total team queue campaign casual matches counted towards rating.
    /// </summary>
    [PHPProperty("cam_cs_amm_team_count")]
    public string TeamCount { get; init; } = statistics.MatchesPlayed.ToString();

    /// <summary>
    ///     Team queue campaign casual rating confidence (0.00 to 1.00).
    /// </summary>
    [PHPProperty("cam_cs_amm_team_conf")]
    public string TeamConfidence { get; init; } = "0.00"; // TODO: Implement Data Tracking

    /// <summary>
    ///     Number of provisional (placement) matches played in team queue campaign casual.
    /// </summary>
    [PHPProperty("cam_cs_amm_team_prov")]
    public string TeamProvisional { get; init; } = "0"; // TODO: Implement Data Tracking

    /// <summary>
    ///     Team queue campaign casual placement set identifier.
    /// </summary>
    [PHPProperty("cam_cs_amm_team_pset")]
    public string TeamPlacementSet { get; init; } = "0"; // TODO: Implement Data Tracking

    /// <summary>
    ///     Total hero kills in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_herokills")]
    public string HeroKills { get; init; } = statistics.HeroKills.ToString();

    /// <summary>
    ///     Total damage dealt to enemy heroes in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_herodmg")]
    public string HeroDamage => Totals.HeroDamage.ToString();

    /// <summary>
    ///     Total experience gained from hero kills in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_heroexp")]
    public string HeroExperience => Totals.HeroExperience.ToString();

    /// <summary>
    ///     Total gold earned from hero kills in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_herokillsgold")]
    public string HeroKillsGold => Totals.GoldFromHeroKills.ToString();

    /// <summary>
    ///     Total hero assists in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_heroassists")]
    public string HeroAssists { get; init; } = statistics.HeroAssists.ToString();

    /// <summary>
    ///     Total deaths in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_deaths")]
    public string Deaths { get; init; } = statistics.HeroDeaths.ToString();

    /// <summary>
    ///     Total gold lost to deaths in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_goldlost2death")]
    public string GoldLostToDeath => Totals.GoldLostToDeath.ToString();

    /// <summary>
    ///     Total seconds spent dead in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_secs_dead")]
    public string SecondsDead => Totals.SecondsDead.ToString();

    /// <summary>
    ///     Total enemy lane creeps killed in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_teamcreepkills")]
    public string TeamCreepKills => Totals.TeamCreepKills.ToString();

    /// <summary>
    ///     Total damage dealt to enemy lane creeps in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_teamcreepdmg")]
    public string TeamCreepDamage => Totals.TeamCreepDamage.ToString();

    /// <summary>
    ///     Total experience gained from enemy lane creeps in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_teamcreepexp")]
    public string TeamCreepExperience => Totals.TeamCreepExperience.ToString();

    /// <summary>
    ///     Total gold earned from enemy lane creeps in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_teamcreepgold")]
    public string TeamCreepGold => Totals.TeamCreepGold.ToString();

    /// <summary>
    ///     Total neutral creeps killed in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_neutralcreepkills")]
    public string NeutralCreepKills => Totals.NeutralCreepKills.ToString();

    /// <summary>
    ///     Total damage dealt to neutral creeps in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_neutralcreepdmg")]
    public string NeutralCreepDamage => Totals.NeutralCreepDamage.ToString();

    /// <summary>
    ///     Total experience gained from neutral creeps in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_neutralcreepexp")]
    public string NeutralCreepExperience => Totals.NeutralCreepExperience.ToString();

    /// <summary>
    ///     Total gold earned from neutral creeps in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_neutralcreepgold")]
    public string NeutralCreepGold => Totals.NeutralCreepGold.ToString();

    /// <summary>
    ///     Total damage dealt to buildings in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_bdmg")]
    public string BuildingDamage => Totals.BuildingDamage.ToString();

    /// <summary>
    ///     Total experience gained from destroying buildings in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_bdmgexp")]
    public string BuildingExperience => Totals.ExperienceFromBuildings.ToString();

    /// <summary>
    ///     Total buildings destroyed in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_razed")]
    public string BuildingsRazed => Totals.BuildingsRazed.ToString();

    /// <summary>
    ///     Total gold earned from destroying buildings in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_bgold")]
    public string BuildingGold => Totals.GoldFromBuildings.ToString();

    /// <summary>
    ///     Total allied creeps denied in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_denies")]
    public string Denies => Totals.Denies.ToString();

    /// <summary>
    ///     Total experience denied to enemies through denies in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_exp_denied")]
    public string ExperienceDenied => Totals.ExperienceDenied.ToString();

    /// <summary>
    ///     Total gold earned in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_gold")]
    public string Gold => Totals.Gold.ToString();

    /// <summary>
    ///     Total gold spent on items in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_gold_spent")]
    public string GoldSpent => Totals.GoldSpent.ToString();

    /// <summary>
    ///     Total experience earned in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_exp")]
    public string Experience => Totals.Experience.ToString();

    /// <summary>
    ///     Total actions (commands issued) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_actions")]
    public string Actions => Totals.Actions.ToString();

    /// <summary>
    ///     Total seconds played in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_secs")]
    public string SecondsPlayed => Totals.SecondsPlayed.ToString();

    /// <summary>
    ///     Total consumable items used in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_consumables")]
    public string Consumables => Totals.ConsumablesPurchased.ToString();

    /// <summary>
    ///     Total wards placed in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_wards")]
    public string Wards { get; init; } = statistics.WardsPlaced.ToString();

    /// <summary>
    ///     Total easy mode campaign casual matches played.
    /// </summary>
    [PHPProperty("cam_cs_em_played")]
    public string EasyModeGamesPlayed { get; init; } = "0"; // TODO: Implement Data Tracking

    /// <summary>
    ///     The account level for campaign casual mode.
    /// </summary>
    [PHPProperty("cam_cs_level")]
    public string CampaignLevel { get; init; } = account.User.TotalLevel.ToString();

    /// <summary>
    ///     The current experience points towards the next campaign casual level.
    /// </summary>
    [PHPProperty("cam_cs_level_exp")]
    public string CampaignLevelExperience { get; init; } = account.User.TotalExperience.ToString();

    /// <summary>
    ///     Total time (in seconds) spent earning experience in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_time_earning_exp")]
    public string TimeEarningExperience => Totals.TimeEarningExperience.ToString();

    /// <summary>
    ///     Total bloodlust kills (first blood) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_bloodlust")]
    public string Bloodlust => Totals.FirstBloods.ToString();

    /// <summary>
    ///     Total double kills (2 kills in quick succession) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_doublekill")]
    public string DoubleKills => Totals.DoubleKills.ToString();

    /// <summary>
    ///     Total triple kills (3 kills in quick succession) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_triplekill")]
    public string TripleKills => Totals.TripleKills.ToString();

    /// <summary>
    ///     Total quad kills (4 kills in quick succession) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_quadkill")]
    public string QuadKills => Totals.QuadKills.ToString();

    /// <summary>
    ///     Total annihilations (5 kills, entire enemy team) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_annihilation")]
    public string Annihilations => Totals.Annihilations.ToString();

    /// <summary>
    ///     Total 3-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks3")]
    public string KillStreak3 => Totals.KillStreak03.ToString();

    /// <summary>
    ///     Total 4-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks4")]
    public string KillStreak4 => Totals.KillStreak04.ToString();

    /// <summary>
    ///     Total 5-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks5")]
    public string KillStreak5 => Totals.KillStreak05.ToString();

    /// <summary>
    ///     Total 6-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks6")]
    public string KillStreak6 => Totals.KillStreak06.ToString();

    /// <summary>
    ///     Total 7-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks7")]
    public string KillStreak7 => Totals.KillStreak07.ToString();

    /// <summary>
    ///     Total 8-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks8")]
    public string KillStreak8 => Totals.KillStreak08.ToString();

    /// <summary>
    ///     Total 9-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks9")]
    public string KillStreak9 => Totals.KillStreak09.ToString();

    /// <summary>
    ///     Total 10-kill streaks in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks10")]
    public string KillStreak10 => Totals.KillStreak10.ToString();

    /// <summary>
    ///     Total 15-kill streaks (immortal) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_ks15")]
    public string KillStreak15 => Totals.KillStreak15.ToString();

    /// <summary>
    ///     Total smackdowns (killing a player on a 3+ kill streak) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_smackdown")]
    public string Smackdowns { get; init; } = statistics.Smackdowns.ToString();

    /// <summary>
    ///     Total humiliations (killing a player with 0 kills) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_humiliation")]
    public string Humiliations => Totals.Humiliations.ToString();

    /// <summary>
    ///     Total nemesis kills (killing the same enemy 3+ times) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_nemesis")]
    public string Nemesis => Totals.Nemeses.ToString();

    /// <summary>
    ///     Total retribution kills (killing your nemesis) in campaign casual matches.
    /// </summary>
    [PHPProperty("cam_cs_retribution")]
    public string Retributions => Totals.Retributions.ToString();

    # endregion

    # region Global Account Statistics

    /// <summary>
    ///     Total disconnections across all game modes.
    /// </summary>
    [PHPProperty("discos")]
    public string TotalDisconnections { get; init; } = aggregates.TotalDisconnections.ToString();

    /// <summary>
    ///     Total matches where disconnection was possible (used for leave percentage calculation).
    /// </summary>
    [PHPProperty("possible_discos")]
    public string PossibleDisconnections { get; init; } = statistics.MatchesPlayed.ToString();

    /// <summary>
    ///     Total games played across all PvP game modes.
    /// </summary>
    [PHPProperty("games_played")]
    public string TotalGamesPlayed { get; init; } = aggregates.TotalGamesPlayed.ToString();

    /// <summary>
    ///     Total bot/practice games won.
    /// </summary>
    [PHPProperty("num_bot_games_won")]
    public string BotGamesWon { get; init; } = aggregates.BotGamesWon.ToString();

    /// <summary>
    ///     Total games played including bot/practice games.
    /// </summary>
    [PHPProperty("total_games_played")]
    public int TotalGamesPlayedInt { get; init; } = aggregates.TotalGamesPlayedIncludingBots;

    /// <summary>
    ///     Total disconnections as an integer value.
    /// </summary>
    [PHPProperty("total_discos")]
    public int TotalDiscosInt { get; init; } = aggregates.TotalDisconnections;

    # endregion

    # region Aggregate Statistics By Game Mode

    /// <summary>
    ///     Total seconds played in public (unranked) matches.
    /// </summary>
    [PHPProperty("acc_secs")]
    public string PublicSecondsPlayed => aggregates.PublicSecondsPlayed.ToString();

    /// <summary>
    ///     Total public (unranked) matches played.
    /// </summary>
    [PHPProperty("acc_games_played")]
    public string PublicGamesPlayed { get; init; } = aggregates.PublicGamesPlayed.ToString();

    /// <summary>
    ///     Total disconnections in public (unranked) matches.
    /// </summary>
    [PHPProperty("acc_discos")]
    public string PublicDisconnections { get; init; } = aggregates.PublicDisconnections.ToString();

    /// <summary>
    ///     Total seconds played in ranked matches.
    /// </summary>
    [PHPProperty("rnk_secs")]
    public string RankedSecondsPlayed => aggregates.RankedSecondsPlayed.ToString();

    /// <summary>
    ///     Total ranked matches played.
    /// </summary>
    [PHPProperty("rnk_games_played")]
    public string RankedGamesPlayed { get; init; } = aggregates.RankedGamesPlayed.ToString();

    /// <summary>
    ///     Total disconnections in ranked matches.
    /// </summary>
    [PHPProperty("rnk_discos")]
    public string RankedDisconnections { get; init; } = aggregates.RankedDisconnections.ToString();

    /// <summary>
    ///     Total seconds played in casual matches.
    /// </summary>
    [PHPProperty("cs_secs")]
    public string CasualSecondsPlayed => aggregates.CasualSecondsPlayed.ToString();

    /// <summary>
    ///     Total casual matches played.
    /// </summary>
    [PHPProperty("cs_games_played")]
    public string CasualGamesPlayed { get; init; } = aggregates.CasualGamesPlayed.ToString();

    /// <summary>
    ///     Total disconnections in casual matches.
    /// </summary>
    [PHPProperty("cs_discos")]
    public string CasualDisconnections { get; init; } = aggregates.CasualDisconnections.ToString();

    /// <summary>
    ///     The total number of MidWars matches played.
    /// </summary>
    [PHPProperty("mid_games_played")]
    public string MidWarsGamesPlayed { get; init; } = aggregates.MidWarsGamesPlayed.ToString();

    /// <summary>
    ///     The total number of disconnections in MidWars matches.
    /// </summary>
    [PHPProperty("mid_discos")]
    public string MidWarsDisconnections { get; init; } = aggregates.MidWarsDisconnections.ToString();

    /// <summary>
    ///     The total number of RiftWars matches played.
    /// </summary>
    [PHPProperty("rift_games_played")]
    public string RiftWarsGamesPlayed { get; init; } = aggregates.RiftWarsGamesPlayed.ToString();

    /// <summary>
    ///     The total number of disconnections in RiftWars matches.
    /// </summary>
    [PHPProperty("rift_discos")]
    public string RiftWarsDisconnections { get; init; } = aggregates.RiftWarsDisconnections.ToString();

    # endregion

    # region Season Information

    /// <summary>
    ///     The highest campaign casual level achieved in the current season.
    /// </summary>
    [PHPProperty("highest_level_current")]
    public string HighestLevelCurrent => account.User.TotalLevel.ToString();

    /// <summary>
    ///     The highest leaderboard ranking achieved in the current season (Immortal rank only).
    ///     Only populated for players with Immortal rank (medal 21+).
    /// </summary>
    [PHPProperty("highest_ranking")]
    public int? HighestRanking => RankExtensions.GetRank(statistics.SkillRating) is Rank.IMMORTAL ? 1 : null; // TODO: Implement Actual Leaderboard Ranking Retrieval

    /// <summary>
    ///     The current campaign casual level in this season.
    /// </summary>
    [PHPProperty("current_level")]
    public string CurrentLevel => account.User.TotalLevel.ToString();

    /// <summary>
    ///     The current leaderboard ranking position (Immortal rank only).
    ///     Only populated for players with Immortal rank (medal 21+).
    /// </summary>
    [PHPProperty("current_ranking")]
    public int? CurrentRanking => RankExtensions.GetRank(statistics.SkillRating) is Rank.IMMORTAL ? 1 : null; // TODO: Implement Actual Leaderboard Ranking Retrieval

    /// <summary>
    ///     The percentage progress towards the next campaign casual level.
    /// </summary>
    [PHPProperty("level_percent")]
    public double LevelPercent { get; init; } = 0.0; // TODO: Implement Data Tracking

    /// <summary>
    ///     The current season identifier.
    /// </summary>
    [PHPProperty("season_id")]
    public string SeasonID { get; init; } = SeasonInformation.CurrentSeasonIndex.ToString();

    /// <summary>
    ///     Total campaign casual games played in previous seasons.
    /// </summary>
    [PHPProperty("prev_seasons_cam_cs_games_played")]
    public int PreviousSeasonsCampaignCasualGamesPlayed { get; init; } = 0; // TODO: Implement Season Tracking

    /// <summary>
    ///     Total disconnections in campaign casual games from previous seasons.
    /// </summary>
    [PHPProperty("prev_seasons_cam_cs_discos")]
    public int PreviousSeasonsCampaignCasualDisconnections { get; init; } = 0; // TODO: Implement Season Tracking

    /// <summary>
    ///     Total campaign casual games played in the latest (most recent completed) season.
    /// </summary>
    [PHPProperty("latest_season_cam_cs_games_played")]
    public string LatestSeasonCampaignCasualGamesPlayed { get; init; } = statistics.MatchesPlayed.ToString();

    /// <summary>
    ///     Total disconnections in campaign casual games from the latest season.
    /// </summary>
    [PHPProperty("latest_season_cam_cs_discos")]
    public string LatestSeasonCampaignCasualDisconnections { get; init; } = statistics.MatchesDisconnected.ToString();

    /// <summary>
    ///     Total campaign casual games played in the current season.
    /// </summary>
    [PHPProperty("curr_season_cam_cs_games_played")]
    public string CurrentSeasonCampaignCasualGamesPlayed { get; init; } = statistics.MatchesPlayed.ToString();

    /// <summary>
    ///     Total disconnections in campaign casual games from the current season.
    /// </summary>
    [PHPProperty("curr_season_cam_cs_discos")]
    public string CurrentSeasonCampaignCasualDisconnections { get; init; } = statistics.MatchesDisconnected.ToString();

    /// <summary>
    ///     Campaign reward data including medal levels and achievement status.
    /// </summary>
    [PHPProperty("con_reward")]
    public CampaignReward CampaignReward { get; init; } = new ();

    # endregion

    # region Favourite Heroes

    /// <summary>
    ///     The icon texture name of the player's most-played hero (the hero identifier with its "Hero_" prefix removed and lower-cased).
    /// </summary>
    [PHPProperty("favHero1")]
    public string FavouriteHero1 => FavouriteHeroes.ElementAtOrDefault(0)?.TextureName ?? string.Empty;

    /// <summary>
    ///     The percentage of the player's matches played with their most-played hero.
    /// </summary>
    [PHPProperty("favHero1Time")]
    public double FavouriteHero1Time => FavouriteHeroes.ElementAtOrDefault(0)?.PlayRatePercentage ?? 0.0;

    /// <summary>
    ///     The internal identifier of the player's most-played hero (e.g., "Hero_Pyromancer").
    /// </summary>
    [PHPProperty("favHero1_2")]
    public string FavouriteHero1Identifier => FavouriteHeroes.ElementAtOrDefault(0)?.Identifier ?? string.Empty;

    /// <summary>
    ///     The icon texture name of the player's second most-played hero.
    /// </summary>
    [PHPProperty("favHero2")]
    public string FavouriteHero2 => FavouriteHeroes.ElementAtOrDefault(1)?.TextureName ?? string.Empty;

    /// <summary>
    ///     The percentage of the player's matches played with their second most-played hero.
    /// </summary>
    [PHPProperty("favHero2Time")]
    public double FavouriteHero2Time => FavouriteHeroes.ElementAtOrDefault(1)?.PlayRatePercentage ?? 0.0;

    /// <summary>
    ///     The internal identifier of the player's second most-played hero.
    /// </summary>
    [PHPProperty("favHero2_2")]
    public string FavouriteHero2Identifier => FavouriteHeroes.ElementAtOrDefault(1)?.Identifier ?? string.Empty;

    /// <summary>
    ///     The icon texture name of the player's third most-played hero.
    /// </summary>
    [PHPProperty("favHero3")]
    public string FavouriteHero3 => FavouriteHeroes.ElementAtOrDefault(2)?.TextureName ?? string.Empty;

    /// <summary>
    ///     The percentage of the player's matches played with their third most-played hero.
    /// </summary>
    [PHPProperty("favHero3Time")]
    public double FavouriteHero3Time => FavouriteHeroes.ElementAtOrDefault(2)?.PlayRatePercentage ?? 0.0;

    /// <summary>
    ///     The internal identifier of the player's third most-played hero.
    /// </summary>
    [PHPProperty("favHero3_2")]
    public string FavouriteHero3Identifier => FavouriteHeroes.ElementAtOrDefault(2)?.Identifier ?? string.Empty;

    /// <summary>
    ///     The icon texture name of the player's fourth most-played hero.
    /// </summary>
    [PHPProperty("favHero4")]
    public string FavouriteHero4 => FavouriteHeroes.ElementAtOrDefault(3)?.TextureName ?? string.Empty;

    /// <summary>
    ///     The percentage of the player's matches played with their fourth most-played hero.
    /// </summary>
    [PHPProperty("favHero4Time")]
    public double FavouriteHero4Time => FavouriteHeroes.ElementAtOrDefault(3)?.PlayRatePercentage ?? 0.0;

    /// <summary>
    ///     The internal identifier of the player's fourth most-played hero.
    /// </summary>
    [PHPProperty("favHero4_2")]
    public string FavouriteHero4Identifier => FavouriteHeroes.ElementAtOrDefault(3)?.Identifier ?? string.Empty;

    /// <summary>
    ///     The icon texture name of the player's fifth most-played hero.
    /// </summary>
    [PHPProperty("favHero5")]
    public string FavouriteHero5 => FavouriteHeroes.ElementAtOrDefault(4)?.TextureName ?? string.Empty;

    /// <summary>
    ///     The percentage of the player's matches played with their fifth most-played hero.
    /// </summary>
    [PHPProperty("favHero5Time")]
    public double FavouriteHero5Time => FavouriteHeroes.ElementAtOrDefault(4)?.PlayRatePercentage ?? 0.0;

    /// <summary>
    ///     The internal identifier of the player's fifth most-played hero.
    /// </summary>
    [PHPProperty("favHero5_2")]
    public string FavouriteHero5Identifier => FavouriteHeroes.ElementAtOrDefault(4)?.Identifier ?? string.Empty;

    # endregion

    # region Upgrades

    /// <summary>
    ///     The custom icon slot ID for the account icon display.
    /// </summary>
    [PHPProperty("slot_id")]
    public string CustomIconSlotID { get; init; } = StatisticsResponseHelper.GetCustomIconSlotID(account);

    /// <summary>
    ///     The list of owned store item codes.
    /// </summary>
    [PHPProperty("my_upgrades")]
    public List<string> OwnedStoreItems { get; init; } = account.User.OwnedStoreItems;

    /// <summary>
    ///     The list of currently selected/equipped store item codes.
    /// </summary>
    [PHPProperty("selected_upgrades")]
    public List<string> SelectedStoreItems { get; init; } = account.SelectedStoreItems;

    /// <summary>
    ///     Detailed information about owned store items, including expiration and discount coupons.
    /// </summary>
    [PHPProperty("my_upgrades_info", isDiscriminatedUnion: true)]
    public Dictionary<string, OwnedStoreItemData> OwnedStoreItemsData { get; init; } = StatisticsResponseHelper.GetOwnedStoreItemsData(account);

    # endregion

    # region Match History

    /// <summary>
    ///     Comma-separated list of recent match IDs.
    /// </summary>
    [PHPProperty("matchIds")]
    public string MatchIDs { get; init; } = string.Empty; // TODO: Implement Data Tracking

    /// <summary>
    ///     Comma-separated list of recent match dates.
    /// </summary>
    [PHPProperty("matchDates")]
    public string MatchDates { get; init; } = string.Empty; // TODO: Implement Data Tracking

    # endregion

    # region Averages

    /// <summary>
    ///     The kill/death/assist ratio string (e.g., "2.5/1.0/3.0").
    /// </summary>
    [PHPProperty("k_d_a")]
    public string KDA { get; init; } = StatisticsResponseHelper.CalculateKDA(statistics);

    /// <summary>
    ///     The average game length in seconds.
    /// </summary>
    [PHPProperty("avgGameLength")]
    public double AverageGameLength => StatisticsResponseHelper.CalculatePerMatchAverage(Totals.SecondsPlayed, statistics.MatchesPlayed);

    /// <summary>
    ///     The average experience earned per minute.
    /// </summary>
    [PHPProperty("avgXP_min")]
    public double AverageExperiencePerMinute => StatisticsResponseHelper.CalculatePerMinuteAverage(Totals.Experience, Totals.TimeEarningExperience);

    /// <summary>
    ///     The average denies per game.
    /// </summary>
    [PHPProperty("avgDenies")]
    public double AverageDenies => StatisticsResponseHelper.CalculatePerMatchAverage(Totals.Denies, statistics.MatchesPlayed);

    /// <summary>
    ///     The average creep kills per game.
    /// </summary>
    [PHPProperty("avgCreepKills")]
    public double AverageCreepKills => StatisticsResponseHelper.CalculatePerMatchAverage(Totals.TeamCreepKills, statistics.MatchesPlayed);

    /// <summary>
    ///     The average neutral creep kills per game.
    /// </summary>
    [PHPProperty("avgNeutralKills")]
    public double AverageNeutralKills => StatisticsResponseHelper.CalculatePerMatchAverage(Totals.NeutralCreepKills, statistics.MatchesPlayed);

    /// <summary>
    ///     The average actions (commands issued) per minute.
    /// </summary>
    [PHPProperty("avgActions_min")]
    public double AverageActionsPerMinute => StatisticsResponseHelper.CalculatePerMinuteAverage(Totals.Actions, Totals.SecondsPlayed);

    /// <summary>
    ///     The average wards placed per game.
    /// </summary>
    [PHPProperty("avgWardsUsed")]
    public double AverageWardsUsed => StatisticsResponseHelper.CalculatePerMatchAverage(Totals.WardsPlaced, statistics.MatchesPlayed);

    # endregion

    # region Miscellaneous

    /// <summary>
    ///     The number of dice tokens (used for dice rolling features).
    /// </summary>
    [PHPProperty("dice_tokens")]
    public string DiceTokens { get; init; } = "100"; // TODO: Implement Data Tracking

    /// <summary>
    ///     The number of game tokens (earnable in-game currency).
    /// </summary>
    [PHPProperty("game_tokens")]
    public int GameTokens { get; init; } = 100; // TODO: Implement Data Tracking

    /// <summary>
    ///     The season level for cosmetic progression (determines seasonal rewards).
    /// </summary>
    [PHPProperty("season_level")]
    public int SeasonLevel { get; init; } = 100; // TODO: Implement Data Tracking

    /// <summary>
    ///     The creep level for creep cosmetics progression.
    /// </summary>
    [PHPProperty("creep_level")]
    public int CreepLevel { get; init; } = 100; // TODO: Implement Data Tracking

    /// <summary>
    ///     The server timestamp in UTC seconds.
    /// </summary>
    [PHPProperty("timestamp")]
    public int ServerTimestamp { get; init; } = Convert.ToInt32(Math.Min(DateTimeOffset.UtcNow.ToUnixTimeSeconds(), Convert.ToInt64(int.MaxValue)));

    /// <summary>
    ///     Quest statistics data (disabled feature).
    /// </summary>
    [PHPProperty("quest_stats")]
    public Dictionary<string, QuestStatistics> QuestStatistics { get; init; } = new () { { "error", new QuestStatistics() } };

    /// <summary>
    ///     The minimum number of matches a free-to-play account must complete to become verified.
    /// </summary>
    [PHPProperty("vested_threshold")]
    public int VestedThreshold => 5;

    /// <summary>
    ///     Success indicator. Set to <see langword="true"/> on successful response.
    /// </summary>
    [PHPProperty(0)]
    public bool Zero => true;

    # endregion
}
