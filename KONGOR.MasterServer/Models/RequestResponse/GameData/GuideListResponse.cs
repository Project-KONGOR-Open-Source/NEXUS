...
public class GuideListResponse
{
    public GuideListResponse(IEnumerable<HeroGuide> guides, Account requestingAccount, int hostTime)
    {
        IEnumerable<string> guidesForResponse = from guide in guides
            ...
            let author = guide.Featured ? string.Empty : guide.Author.Clan is null ? guide.Author.Name : $"[{guide.Author.Clan.Tag}]" + guide.Author.Name
            ...
            select new StringBuilder().Append($"{guide.ID}").Append('|')
                ...
                .Append(new List<string> { "not_def", "is_def" }.First()).Append('|') // TODO: Implement Support For Default Guide
                .Append(new List<string> { "not_fav", "is_fav" }.First()).Append('|') // TODO: Implement Support For Favourite Guide
