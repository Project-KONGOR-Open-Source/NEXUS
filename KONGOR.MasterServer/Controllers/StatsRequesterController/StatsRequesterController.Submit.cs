...
public partial class StatsRequesterController
{
    private async Task<IActionResult> HandleStatsSubmission(StatsForSubmissionRequestForm form)
    {
        ...
        if (matchServer is not null) Logger.LogInformation($"[StatsSubmission] Session {form.Session} Found In Redis. Server ID: {matchServer.ID}");

        if (matchServer is null)
        {
            Logger.LogWarning($"[StatsSubmission] Session {form.Session} Not Found In Redis. Attempting Database Fallback.");
            ...
            if (account != null)
            {
                Logger.LogInformation($"[StatsSubmission] Session {form.Session} Found In Database (User: {account.Name}). Recovering Context.");
                ...
                else
                {
                    ...
                    if (form.MatchStats.TryGetValue("server_id", out string? serverIdStr) && int.TryParse(serverIdStr, out int parsedServerId))
                    {
                    ...
                    else
                    {
                        ...
                        serverID = 0; 
                        Logger.LogWarning($"Match Start Data Missing & Server ID Not In Payload For Match {matchID}. Defaulting to Server ID 0.");
                    }
                }
                ...
                Logger.LogInformation($"Recovered Session Context From Database For User {account.Name} In Match {matchID}.");
            }
        }

        if (matchServer is null)
        {
            Logger.LogError($"[StatsSubmission] Session {form.Session} Not Found In Database. Triggering Ultimate Fallback (Orphaned Session).");
            Logger.LogWarning($@"Session ""{form.Session}"" Not Found During Stats Submission. Database Fallback Failed. Forcing Success (Orphaned Session) To Prevent Client Logout.");
