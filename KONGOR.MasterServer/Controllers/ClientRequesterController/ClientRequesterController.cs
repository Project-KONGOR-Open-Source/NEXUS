    ...
    ILogger<ClientRequesterController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> ClientRequester()
    {
        ...
        Logger.LogInformation(
            $"[ClientRequester] Processing '{functionName}'. Raw Cookie: '{cookieRaw}' (RequiresValidation: {endpointRequiresCookieValidation})");
        ...
        if (endpointRequiresCookieValidation.Equals(true) && accountSessionCookieIsValid.Equals(false))
        {
            ...
            if (account != null)
            {
                ...
                Logger.LogInformation(
                    $"[ClientRequester] DB Hit! Restored Session From Database For Cookie '{cookie}' (User: {account.Name})");
            }
            else
            {
                Logger.LogError($"[ClientRequester] DB Miss! Cookie '{cookie}' not found in Accounts table.");
                Logger.LogWarning(
                    $@"IP Address ""{Request.HttpContext.Connection.RemoteIpAddress?.MapToIPv4().ToString() ?? "UNKNOWN"}"" Has Made A Client Request With Forged Cookie ""{cookieRaw}""");
