...
/// <summary>
///     Smoke tests verifying that the TRANSMUTANSTEIN host builds end-to-end against real SQL Server and distributed cache containers, migrations run successfully against the per-test database, and the distributed cache is reachable.
...
public sealed class HostedServicesSmokeTests(TRANSMUTANSTEINIntegrationWebApplicationFactory webApplicationFactory)
{
    ...
    [Test]
    public async Task DistributedCache_Multiplexer_Resolves()
    {
