...
4. **Given** a player with an active session, **When** the same account attempts to connect from a different location, **Then** the server detects the duplicate session and enforces the account sharing policy
...
4. **Given** multiple rating types (Normal, Casual, Midwars), **When** calculating match compatibility, **Then** the broker uses the appropriate rating for the selected game mode

---

### Edge Cases

- What happens when a player's connection drops unexpectedly during a match?
- How does the system handle rapid reconnection attempts (connection spam)?
- What happens when channel buffer limits are exceeded (16KB limit)?
- How does the system prevent chat flooding and spam?
- What happens when game servers become unavailable during matchmaking?
- How does the system handle authentication session expiration?
- What happens when a player attempts to join more channels than the allowed limit?
- How does the system handle clan channel access when a player leaves a clan mid-session?
- What happens when matchmaking queue wait times exceed acceptable thresholds?
- How does the system handle time zone differences for scheduled tournaments?
...
- **FR-001**: System MUST support three separate TCP listener ports for client connections, game server connections, and server manager connections
...
- **FR-004**: System MUST track last communication timestamp for each connection and remove stale connections
...
- **FR-007**: System MUST detect and enforce account sharing policies for concurrent connections from the same account
...
- **FR-037**: System MUST support multiple rating types per player: Normal TMR, Casual TMR, Midwars TMR, Riftwars TMR, Campaign Normal TMR, Campaign Casual TMR, Reborn Normal TMR, Reborn Casual TMR, Midwars Reborn TMR
...
- **FR-057**: System MUST maintain separate statistics per game mode (Normal, Casual, Midwars, etc.)
...
- **FR-063**: System MUST enforce account bans and suspensions during authentication
...
- **ChatSession**: Represents a connected client with authentication state, account information, chat mode, channels, and matchmaking group membership
- **ChatChannel**: Represents a chat channel with members, admin levels, topic, password, flags (permanent, hidden, clan-specific), and moderation state
- **ChatChannelMember**: Represents a player's membership in a channel with admin level and silence status
- **MatchmakingGroup**: Represents a team preparing to queue with members, leader, ready states, loading states, and queue status
- **MatchmakingGroupMember**: Represents a player in a matchmaking group with team slot assignment and status flags
- **GameServer**: Represents a connected game server with availability status, capacity, current match count, and region
- **PlayerStatistics**: Represents player ratings and match history across multiple game modes with separate TMR values
- **Buddy**: Represents a friend relationship between two players with online status tracking
- **Clan**: Represents a clan organization with members, ranks, and associated channels
- **ClanMember**: Represents a player's membership in a clan with role and join date
...
- **SC-009**: All 200+ protocol commands defined in ChatProtocol.cs have implemented handlers
