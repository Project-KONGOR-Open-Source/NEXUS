...
/// <summary>
///     Helper for building SRP-authenticated state in integration tests: creates accounts with valid SRP credentials, drives the full authentication handshake against the HTTP pipeline, and exposes the intermediate pre-authentication stage for tests that need to interrogate it directly.
/// </summary>
public sealed class SRPAuthenticationService(KONGORIntegrationWebApplicationFactory webApplicationFactory)
{
    /// <summary>
    ///     Creates an account with valid SRP credentials for testing.
    /// </summary>
    public async Task<(Account Account, string Password)> CreateAccountWithSRPCredentials(string emailAddress, string accountName, string password)
    {
        ...
        string srpPasswordHash = SRPPasswordHasher.ComputeSRPPasswordHash(password, salt);
    ...
    /// <summary>
    ///     Creates an account and then performs the complete SRP authentication flow against it.
    /// </summary>
    public async Task<SRPAuthenticationData> AuthenticateWithSRP(string emailAddress, string accountName, string password)
    {
    ...
    public async Task<SRPAuthenticationData> PerformFullAuthentication(Account account, string password)
    {
        ...
        HttpResponseMessage preAuthResponse = await httpClient.PostAsync("client_requester.php?f=pre_auth", preAuthContent);
        ...
        {
            IDictionary<object, object>? errorDictionary = PhpSerialization.Deserialize(preAuthResponseBody) as IDictionary<object, object>;

            string errorMessage = errorDictionary?["auth"] as string ?? $"Pre-Authentication Failed With HTTP {(int)preAuthResponse.StatusCode}";
        ...
        IDictionary<object, object>? preAuthData = PhpSerialization.Deserialize(preAuthResponseBody) as IDictionary<object, object>
        ...
        string passwordSalt = preAuthData["salt2"] as string ?? throw new NullReferenceException("Password Salt Is NULL");
        string serverPublicEphemeral = preAuthData["B"] as string ?? throw new NullReferenceException("Server Public Ephemeral Is NULL");
        ...
        SrpSession clientSession = client.DeriveSession(clientEphemeral.Secret, serverPublicEphemeral, sessionSalt, account.Name, privateClientKey);
        ...
        {
            IDictionary<object, object>? errorDictionary = PhpSerialization.Deserialize(authResponseBody) as IDictionary<object, object>;

            string errorMessage = errorDictionary?["auth"] as string ?? $"Authentication Failed With HTTP {(int) authResponse.StatusCode}";
        ...
        string serverProof = stageTwoData["proof"] as string ?? throw new NullReferenceException("Server Proof Is NULL");
    ...
    /// <summary>
    ///     Performs the pre-authentication stage only.
    /// </summary>
    public async Task<(bool Success, string? SessionSalt, string? PasswordSalt, string? ServerPublicEphemeral, string? ErrorMessage)> PerformPreAuthentication(string accountName, string clientPublicEphemeral)
    {
        HttpClient httpClient = webApplicationFactory.CreateClient();
        ...
        {
            IDictionary<object, object>? errorDictionary = PhpSerialization.Deserialize(responseBody) as IDictionary<object, object>;
        ...
        IDictionary<object, object>? preAuthData = PhpSerialization.Deserialize(responseBody) as IDictionary<object, object>
            ?? throw new NullReferenceException("Pre-Authentication Response Deserialised To NULL");
