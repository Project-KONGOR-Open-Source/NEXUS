...
public class ASPIRE
{
    public static void Main(string[] args)
    {
        ...
        string chatServerProtocol = builder.Configuration.GetRequiredSection("ChatServer").GetValue<string?>("Protocol")
            ?? throw new NullReferenceException("Chat Server Protocol Is NULL");
        ...
        builder.AddProject<KONGOR_MasterServer>("master-server", builder.Environment.IsProduction() ? "KONGOR.MasterServer Production" : "KONGOR.MasterServer Development")
            ...
            .WithEnvironment("CHAT_SERVER_PROTOCOL", chatServerProtocol).WithEnvironment("CHAT_SERVER_ADDRESS", chatServerAddress).WithEnvironment("CHAT_SERVER_PORT", chatServerPort.ToString());
