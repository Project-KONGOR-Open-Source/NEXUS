...
///     This path complements <see cref="GroupPlayerLoadingStatus"/> which auto-joins the queue when all players reach 100% loading status.
...
public class GroupJoinQueue : ISynchronousCommandProcessor<ChatSession>
{
    public void Process(ChatSession session, ChatBuffer buffer)
    {
        ...
        {
            Log.Error("[BUG] Account ID {AccountID} Attempted To Join Queue For Group {GroupGUID} But Is Not The Leader", session.Account.ID, group.GUID);
