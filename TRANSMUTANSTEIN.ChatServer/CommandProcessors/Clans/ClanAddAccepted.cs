    ...
    : IAsynchronousCommandProcessor<ChatSession>
{
    public async Task Process(ChatSession session, ChatBuffer buffer)
    {
        try
        {
            ...
            Log.Information("[ClanAddAccepted] START processing for Account {AccountID} ({Name})", account.ID,
                account.Name);
            ...
            else if (inviteKey != null)
            {
                ...
                if (invite is null)
                {
                    Log.Warning("[ClanAddAccepted] Invite Key {Key} found but Convert failed (Race Condition?)",
                        inviteKey);
                    return;
                }
                ...
                Log.Information("[ClanAddAccepted] Clan Found: {ClanName} ({Tag}). Members: {Count}", clan.Name,
                    clan.Tag, clan.Members.Count);
    ...
    private async Task CreateClan(MerrickContext merrick, PendingClan pendingClan)
    {
        ...
        List<int> allMemberIds = new() { pendingClan.CreatorAccountId };
        ...
        if (creatorSession != null)
        {
            ...
            creatorSession.Send(new ClanNewMemberResponse(pendingClan.CreatorAccountId, newClan.ID,
                pendingClan.ClanName, pendingClan.ClanTag));
        }
        ...
        foreach (int memberId in pendingClan.MembersAccountId)
        {
            ...
            if (memberSession != null)
            {
                ...
                if (creatorSession != null)
                {
                    creatorSession.Send(new ClanNewMemberResponse(memberId, newClan.ID, pendingClan.ClanName,
                        pendingClan.ClanTag));
                }

                // Tell member about themselves and clan
                memberSession.Send(new ClanNewMemberResponse(memberId, newClan.ID, pendingClan.ClanName,
                    pendingClan.ClanTag));
                ...
                memberSession.Send(new ClanNewMemberResponse(pendingClan.CreatorAccountId, newClan.ID,
                    pendingClan.ClanName, pendingClan.ClanTag));
            }
        }
