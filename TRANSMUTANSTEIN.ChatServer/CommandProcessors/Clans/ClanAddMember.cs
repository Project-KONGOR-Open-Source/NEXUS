    ...
    : IAsynchronousCommandProcessor<ChatSession>
{
    public async Task Process(ChatSession session, ChatBuffer buffer)
    {
        ...
        {
            Log.Warning("[CLAN-INVITE-DEBUG] FAIL_ONLINE: Target Username '{Name}' NOT FOUND in Database.",
                targetUsername);
        ...
        if (targetSession == null || targetSession.ClientMetadata.LastKnownClientState <
            ChatProtocol.ChatClientStatus.CHAT_CLIENT_STATUS_CONNECTED)
        {
            Log.Warning(
                "[CLAN-INVITE-DEBUG] FAIL_ONLINE: Target {TargetID} ({TargetName}) not found in active sessions or not connected. Session Null? {SessionNull}. State: {State}",
                targetAccount.ID, targetAccount.Name, targetSession == null,
                targetSession?.ClientMetadata.LastKnownClientState);
            session.Send(new ClanCreateFailResponse(ChatProtocol.Command.CHAT_CMD_CLAN_ADD_FAIL_ONLINE));
            return;
        }
