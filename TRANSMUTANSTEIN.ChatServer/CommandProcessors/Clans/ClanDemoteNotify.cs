...
public class ClanDemoteNotify(MerrickContext merrick, IChatContext chatContext) : IAsynchronousCommandProcessor<ChatSession>
{
    public async Task Process(ChatSession session, ChatBuffer buffer)
    {
        ...
        {
            return; // Strict hierarchy check
        }

        // If Target is Member, cannot demote further?
        if (targetAccount.ClanTier == ClanTier.None || targetAccount.ClanTier == ClanTier.Member)
        {
            return;
        }
