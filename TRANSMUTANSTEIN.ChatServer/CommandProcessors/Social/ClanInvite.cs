...
public class ClanInvite : ISynchronousCommandProcessor<ClientChatSession>
{
    public void Process(ClientChatSession session, ChatBuffer buffer)
    {
        ...
        if (targetSession.Metadata.ClientChatModeState is ChatProtocol.ChatModeType.CHAT_MODE_DND)
        {
            ...
            autoResponse.WriteInt8(Convert.ToByte(ChatProtocol.ChatModeType.CHAT_MODE_DND)); // Current Chat Mode
            autoResponse.WriteString(targetSession.Account.Name);                            // Target Account Name
            autoResponse.WriteString(targetSession.Metadata.ClientChatModeReason);           // Chat Mode Reason
