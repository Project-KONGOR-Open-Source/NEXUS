...
public static class MatchInvite
{
    ...
    public static void Send(ClientChatSession senderSession, ClientChatSession? targetSession)
    {
        ...
        invite.WriteString(senderSession.Account.Name);                                // Account Name
        invite.WriteInt32(senderSession.Account.ID);                                   // Account ID
        invite.WriteInt8(Convert.ToByte(senderSession.Metadata.LastKnownClientState)); // Chat Client Status
        invite.WriteInt8(senderSession.Account.GetChatClientFlags());                  // Chat Client Flags
        invite.WriteString(senderSession.Account.NameColourNoPrefixCode);              // Account Name Colour
        invite.WriteString(senderSession.Account.IconNoPrefixCode);                    // Account Icon
