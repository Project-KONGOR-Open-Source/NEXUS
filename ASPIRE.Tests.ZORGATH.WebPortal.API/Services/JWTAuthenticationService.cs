...
/// <summary>
///     Helper for driving the ZORGATH web portal's user registration and login flow end-to-end in integration tests.
///     Produces a fully-authenticated user (registered email, registered account, and a valid JWT) so that test cases can focus on the behaviour under test.
/// </summary>
public sealed class JWTAuthenticationService(ZORGATHIntegrationWebApplicationFactory webApplicationFactory)
{
    /// <summary>
    ///     Completes the full authentication flow: email registration, user registration, and login.
    /// </summary>
    ...
    private async Task<Token> RegisterEmailAddress(string emailAddress)
    {
        ...
        Token? token = await merrickContext.Tokens.SingleOrDefaultAsync(tokenRecord =>
            tokenRecord.EmailAddress.Equals(emailAddress) && tokenRecord.Purpose.Equals(TokenPurpose.EmailAddressVerification));
