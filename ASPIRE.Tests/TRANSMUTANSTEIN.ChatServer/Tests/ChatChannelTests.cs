...
public sealed class ChatChannelTests
{
    ...
    public async Task JoinChannel_Success()
    {
        ...
        ChatBuffer joinBuffer = new ChatBuffer();
    ...
    public async Task SendMessage_BroadcastsToChannel()
    {
        ...
        // Client 1 Joins
        {
            ChatBuffer join1 = new ChatBuffer();
            ...
            int read1 = 0;
            while (read1 < py1.Length) read1 += await client1.GetStream().ReadAsync(py1, read1, py1.Length - read1);
            ChatBuffer b1 = new ChatBuffer(py1);
            b1.ReadString(); // Name
        ...
        // Client 2 Joins
        {
            ChatBuffer join2 = new ChatBuffer();
            ...
            int read2 = 0;
            while (read2 < py2.Length) read2 += await client2.GetStream().ReadAsync(py2, read2, py2.Length - read2);
            ...
            await client1.GetStream().ReadExactlyAsync(h1_notif);
             ...
             int read1_notif = 0;
             while (read1_notif < py1_notif.Length) read1_notif += await client1.GetStream().ReadAsync(py1_notif, read1_notif, py1_notif.Length - read1_notif);
        }
        
        // Act - Client 1 Sends Message
        string messageText = "Hello World";
        ChatBuffer msgBuffer = new ChatBuffer();
        ...
        int r_msg = 0;
        while (r_msg < py_msg.Length) r_msg += await client2.GetStream().ReadAsync(py_msg, r_msg, py_msg.Length - r_msg);
        
        ChatBuffer msgPayload = new ChatBuffer(py_msg);
    ...
    public async Task LeaveChannel_Success()
    {
        ...
        int channelId = 0;
        {
            ChatBuffer join = new ChatBuffer();
            ...
            await stream.WriteAsync(r.ToArray());
             ...
             int rd = 0;
             while (rd < py.Length) rd += await stream.ReadAsync(py, rd, py.Length - rd, cts.Token);
             ChatBuffer b = new ChatBuffer(py);
             b.ReadString();
             channelId = b.ReadInt32();
        }
        
        // Act - Leave
        ChatBuffer leaveBuffer = new ChatBuffer();
        ...
        using CancellationTokenSource ctsLeave = new CancellationTokenSource(TimeSpan.FromSeconds(5));
