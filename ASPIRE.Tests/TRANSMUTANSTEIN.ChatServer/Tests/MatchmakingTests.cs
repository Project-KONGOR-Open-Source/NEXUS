...
public class MatchmakingTests
{
    ...
    public async Task PopularityUpdate_ReturnsConfiguredModes()
    {
        ...
        MatchmakingConfiguration mockConfig = new MatchmakingConfiguration
        {
            Ranked = new MatchmakingMapConfiguration
            {
                Map = "mock_ranked",
                Modes = ["ap", "sd"],
                Regions = ["USE"],
                Match = new MatchConfiguration 
                { 
                    MaximumPlayerRatingDifference = 500, 
                    IsRanked = true, 
                    TeamSize = 5 
                }
            },
            Unranked = new MatchmakingMapConfiguration
            {
                ...
                Match = new MatchConfiguration 
                { 
                    MaximumPlayerRatingDifference = 0, 
                    IsRanked = false, 
                    TeamSize = 5 
                }
            },
        ...
        using TcpClient client = await ChatTestHelpers.ConnectAndAuthenticateAsync(app, TestPort, 9999, "MatchmakingTester");
        NetworkStream stream = client.GetStream();

        // 4. Send Popularity Update Request
        ChatBuffer requestBuffer = new ChatBuffer();
        ...
        while (!cts.Token.IsCancellationRequested)
        {
            ...
            {
                ChatBuffer reader = new ChatBuffer(payload);
