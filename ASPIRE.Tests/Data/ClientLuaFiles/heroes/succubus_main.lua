...
function object:SkillBuild()
	...
	for i = nLevel, nLevel + nLevelPoints do
...
function behaviorLib.CustomRetreatExecute(botBrain)
	...
	local nMesmerizeRange = skills.mesmerize:GetRange()
	local nHeartacheRange = skills.heartache:GetRange()
	local bMesmerizeCanActivate = skills.mesmerize:CanActivate()
		...
		if nLastRetreatUtil > object.retreatCastThreshold then
			for _, hero in pairs(core.localUnits["EnemyHeroes"]) do
				if not hero:IsMagicImmune() then
					...
					if bMesmerizeCanActivate and nDistanceSQ < nMesmerizeRange * nMesmerizeRange then
						bActionTaken = core.OrderAbilityEntity(botBrain, skills.mesmerize, hero)
						break
					end
				end
			end
...
function object:oncombateventOverride(EventData)
	...
	if EventData.Type == "Ability" then
		...
		elseif EventData.InflictorName == "Ability_Succubis3" then
			nAddBonus = nAddBonus + object.nMesmerizeUseBonus
...
local function CustomHarassUtilityFnOverride(hero)
	...
	if skills.mesmerize:CanActivate() then
		nVal = nVal + object.nMesmerizeUpBonus
	end
...
local function HarassHeroExecuteOverride(botBrain)
	...
	local nMesmerizeRange = skills.mesmerize:GetRange()
	local nSmittenRange = skills.smitten:GetRange()
	local bMesmerizeCanActivate = skills.mesmerize:CanActivate()
	local bSmittenCanActivate = skills.smitten:CanActivate()

	--teamfight
	if not bActionTaken then
		if nLastHarassUtility > object.nMesmerizeThreshold then
			for _,hero in pairs(core.localUnits["EnemyHeroes"]) do
				if hero ~= unitTarget then
					if not hero:HasState("State_Succubis_Ability3") and not hero:HasState("State_Succubis_Ability1") and not hero:IsMagicImmune() then
						...
						if bMesmerizeCanActivate and nDistanceSQ < nMesmerizeRange * nMesmerizeRange then
							...
							break
						--smitten
						elseif bSmittenCanActivate and nDistanceSQ < nSmittenRange * nSmittenRange then
...
function behaviorLib.healHeartacheExecute(botBrain)
	...
	if not bActionTaken and bHeartacheCanActivate then
		...
		if core.NumberElements(core.localUnits["EnemyHeroes"]) > 0 then
			...
			if core.NumberElements(core.localUnits["EnemyCreeps"]) > 0 then
...
function behaviorLib.newPickRuneExecute(botBrain)
	...
	if core.NumberElements(core.localUnits["EnemyHeroes"]) > 0 then
		...
		if skills.mesmerize:CanActivate() then
			for _,hero in pairs(core.localUnits["EnemyHeroes"]) do
				if Vector3.Distance2DSq(mypos, hero:GetPosition()) <= nMesmerizeRange * nMesmerizeRange then
					bActionTaken = core.OrderAbilityEntity(botBrain, skills.mesmerize, hero)
				end
			end
		end
