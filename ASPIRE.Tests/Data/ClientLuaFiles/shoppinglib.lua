...
shoppingLib.nBuyInterval = 50 -- One Shopping Round per Behavior utility call 
...
function  shoppingLib.Autobuy()
	...
	if tConsumables[sNameHomecomingStone] then
		--only buy stones if we do not have Post Haste
...
function shoppingLib.SortItems (unitSelected)
	...
	--item slot list	
	local tSlots = 		{false, false, false, false, false, false, false, false}
...
function shoppingLib.SellItems (nNumber, unitSelected, tRestrictionSlotTable)
	...
	if tRestrictionSlotTable then
		tAccess = {false,false,false,false,false,false,false,false,true,true,true,true,true,true}
		for _, tEntry in pairs(tRestrictionSlotTable) do
			nSlot = tEntry[1] or tEntry -- SPecial case for courier-slots 
			tAccess[nSlot] = true
		end
	else
		tAccess = {true,true,true,true,true,true,true,true,true,true,true,true,true,true}
	end
	
	--index items
	for nSlot=1, 14, 1 do
		if tAccess[nSlot] then
			local tCurrentInventory = nSlot < 9 and tInventory or tStash
	...
	while nNumber > 0 do
		...
		if nSellingSlot then
			...
			local toSellItem = core.unitSelf:GetInventory(nSellingSlot <= 8)[nSellingSlot]
			if toSellItem then
				behaviorLib.removeItemBehavior(toSellItem:GetName())
			end
			if nSellingSlot <= 8 then
				bStashOnly = false
		...
		if nGoldAmtBefore >= nItemCost then
			...
			if nOpenSlotsAccessable < 1 then
				...
				if bGoldReduced then 
					...
					behaviorLib.addItemBehavior(itemNextDef:GetName())
