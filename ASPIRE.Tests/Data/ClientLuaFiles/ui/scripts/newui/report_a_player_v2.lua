...
-------------------
-- In game stuff --
function Rap_Info.ShowRapMenu(self, playerName, heroIcon)
	...
	GetWidget('RAP_pop_hero_icon', interfaceGameName):SetTexture(heroIcon)
...
function Rap_Info.UpdateIndividualEntries(deltaTime)
	...
	for playerName, rapInfo in pairs(Rap_Info.Storage.ReportedPlayers) do
		...
		if(dontAllowSubmit) then
			...
			local descR = GetWidget('rap_postgame_entry_description_' .. playerName):GetInputLine()
			if (tostring(descR) ~= Translate('rap_final_error_1')) and string.len(descR) >= 1 then
				...
				interface:UICmd("Trigger('rap_submission_processing', '" .. Translate('rap_require_entry').."');")
				GetWidget('rap_postgame_submit_buton', interfaceMainName):SetEnabled(false)
			end
		end
