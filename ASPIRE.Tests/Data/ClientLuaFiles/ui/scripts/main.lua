...
local function LoginStatus(self, accountStatus, statusDescription, isLoggedIn, pwordExpired, isLoggedInChanged, updaterStatus)
	...
	if (isLoggedIn) then
		if ( (isLoggedInChanged) or (HoN_Main.uiLoginState) )  and ( (updaterStatus == 5) or (updaterStatus == 6) or (updaterStatus == 8) or (updaterStatus == 8 and GetCvarBool('upd_requireUpdate')) ) then
			LoggedIn()
