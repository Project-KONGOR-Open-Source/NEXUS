...
println('=============================================')
println('======== DIRECT OVERRIDE WORKING | LIVE RELOAD TEST============')
println('======== HELLO PROJECT KONGOR ============')
println('=============================================')
...
local function SlideInPanel(widget, offset)
	offset = offset or '0'
	widget:SetVisible(true)
	if GetCvarBool('cg_menuTransitions') then
		widget:Sleep(200, function() widget:SlideY(offset, 300) end)
	else
		widget:SetY(offset or '0')
	end
end
...
local function ShowPlayerStats()
	GetWidget('playerstats', 'main'):FadeIn(250)
end

local function HidePlayerStats()
	GetWidget('playerstats_switch_accounts_list', 'main'):FadeOut(150)
	GetWidget('playerstats', 'main'):FadeOut(150)
end

local function ShowMatchStats()
	GetWidget('match_stats', 'main'):FadeIn(250)
end

local function HideMatchStats()
	GetWidget('match_stats', 'main'):FadeOut(150)
end

local function ShowMapSelection()
	MapSelection_V2:Show()
end

local function HideMapSelection()
	MapSelection_V2:Hide()
end

local function ShowHostedGame()
	Player_Hosted_V2:Show()
end

local function HideHostedGame()
...
local function GetMainMenuPanels()
	...
	HoN_Main.menuPanels.table['playerstats'] 				= { ['show'] = ShowPlayerStats, ['hide'] = HidePlayerStats, ['effectid'] = '1', ['context'] = 'playerstats' }
	...
	HoN_Main.menuPanels.table['player_motd'] 				= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanelTall, ['effectid'] = '1', ['showcmd'] = "Call('com_hider2', 'FadeIn(200)'); Call('main_bottom_bottom', 'SlideY(370i, 800)'); Call('main_bottom_bottom2', 'SlideY(370i, 800)');", ['hidecmd'] = "Call('main_bottom_bottom', 'SlideY(0, 800)'); Call('main_bottom_bottom2', 'SlideY(0, 800)');", ['hidefunction'] = function() interface:GetWidget('com_hider2'):FadeOut(200) end }
	...
	HoN_Main.menuPanels.table['game_options'] 				= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanel, ['effectid'] = '9', ['finishpanel'] = "options_referral_browser", ['finishcmd'] = "DoEvent();", ['offset'] = '1.5h' }
	HoN_Main.menuPanels.table['change_log'] 				= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanel, ['effectid'] = '11', ['offset'] = '2.5h' }
	...
	HoN_Main.menuPanels.table['wareffort_new'] 				= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanel, ['context'] = 'wareffort_new', ['effectid'] = '1' }
	...
	HoN_Main.menuPanels.table['retrieve_password'] 			= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanel, ['effectid'] = '15', ['offset'] = '13h' }
	HoN_Main.menuPanels.table['form_create_paid_account'] 	= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanel, ['offset'] = '13h' }
	...
	HoN_Main.menuPanels.table['rap_status']					= { ['show'] = SlideInPanel, ['hide'] = SlideOutPanel, ['offset'] = '2.5h'}

	-- HoN_Main.menuPanels.table['map_selection']				= { ['show'] = ShowMapSelection, ['hide'] = HideMapSelection}
	-- HoN_Main.menuPanels.table['hosted_game']				= { ['show'] = ShowHostedGame, ['hide'] = HideHostedGame}
...
local function LoggedOut()
	...
	GetWidget('motd'):SetVisible(false)
end
...
local function LoggedIn()
	...
	Cvar.CreateCvar('ui_match_is_spectating', 'bool', 'false')
...
local function questsDisabledUpdate()
	...
	if questsLastDisabled or (not chatLastConnected) then
		GetWidget('midbar_button_wareffort'):SetEnabled(false)
		--GetWidget('midbar_button_wareffort'):SetCallback('onmouseover', function() Common_V2:ShowSimpleTip(true, 'quests_disabled', '18h') end) --todo
        --GetWidget('midbar_button_wareffort'):SetCallback('onmouseout', function() Common_V2:ShowSimpleTip(false) end) --todo
		HoN_Main:SelectChat()
	else
		GetWidget('midbar_button_wareffort'):SetEnabled(true)
		--todo
		--GetWidget('midbar_button_wareffort'):ClearCallback('onmouseover') --todo
		--GetWidget('midbar_button_wareffort'):ClearCallback('onmouseout') --todo
	end
	
	--todo
	--GetWidget('midbar_button_wareffort'):RefreshCallbacks() --todo
...
interface:RegisterWatch('ShowConfirmSwitchSpectatorDialog', function(_)
	Trigger('TriggerDialogBox', ' ', 'general_ok', 'general_cancel', '', '', '', 'server_switch_viewstreaming_text', 'Disconnect() ConfirmStreamSwitch()', 'AbortStreamSwitch()')
end)

interface:RegisterWatch('ShowConfirmStopReplayDialog', function(_)
	Trigger('TriggerDialogBox', ' ', 'general_ok', 'general_cancel', '', '', '', 'server_stop_replay_text_for_connect', 'Disconnect() ConfirmStopReplayForConnect()', 'AbortStopReplayForConnect()')
end)

interface:RegisterWatch('StreamAbnormallyStopped', function(_, param)
	...
	Trigger('TriggerDialogBox', 'watch_system_warning_title', '', 'general_ok', '', '', '', label, '', '')
end)
...
local function LoginStatus(self, accountStatus, statusDescription, isLoggedIn, pwordExpired, isLoggedInChanged, updaterStatus)
	...
	if (isLoggedIn) then
		if ( (isLoggedInChanged) or (HoN_Main.uiLoginState) )  and ( (updaterStatus == 5) or (updaterStatus == 6) or (updaterStatus == 8 and GetCvarBool('upd_requireUpdate')) ) then
			LoggedIn()
			...
			-- walkthrough prompt
			if (Client.AccountLessThan24HoursOld() and (not Client.IsSubAccount())) then
				if (not GetDBEntry('newTutorialPrompted'..GetAccountName())) then
					GetDBEntry('newTutorialPrompted'..GetAccountName(), true, true)
					UIManager.GetInterface('main'):HoNMainF('MainMenuPanelToggle', 'main_splash_new_player', nil, nil, nil, nil, 63)

					-- Also populate the MM bots
					HoN_Matchmaking:FillTeamWithBots()
					HoN_Matchmaking:ChangeBotDifficulty(1)
					HoN_Matchmaking:SelectTab("coop")
				end
			end
		end
	elseif (isLoggedInChanged) then
		LoggedOut()
		if (HoN_IM_Panel) then
			HoN_IM_Panel:OnLogout()
			Trigger("TMMReset")
		end

		if Communicator_V2 then
			Communicator_V2:OnLogout()
		end
	end
...
local function MainMenuButtonsHandler(disableAll)
	...
	if (disableAll) then
		...
		GetWidget('main_playerinfo_play'):SetEnabled(false)
	...
	elseif (gamephase == 0) then
		if (IsInQueue() or IsInScheduledMatch()) then 		
			GetWidget('midbar_button_hontour'):SetEnabled(false)
			GetWidget('midbar_button_matchmaking'):SetEnabled(false)
			GetWidget('main_playerinfo_play'):SetEnabled(false)
		else
			GetWidget('midbar_button_hontour'):SetEnabled(IsTMMEnabled())
			GetWidget('midbar_button_matchmaking'):SetEnabled(true)
			GetWidget('main_playerinfo_play'):SetEnabled(true)
		end
	...
	if gamephase ~= 0 then
		...
		GetWidget('main_playerinfo_play'):SetEnabled(isButtonEnabled)
	end
...
local function CheckForAutoLogin()
	if GetCvarBool('login_rememberPassword') and (not IsLoggedIn()) and (not IsLoggingIn()) and GetWidget('main_login_password') and GetAgreePrivacyPolicy() then
...
local function UpdaterStatus(self, status)
	Echo('UpdaterStatus status: '..status)
	local status = tonumber(status)
	if (status == 3) or (status == 7) then
		GetWidget('patch_updater'):SlideY('135i', 300)
		GetWidget('login_gear'):Rotate(73, 300)
	else
		GetWidget('patch_updater'):SlideY('535i', 300)
				...
				if (HoN_Main.menuPanels.table[targetPanel]) and (HoN_Main.menuPanels.table[targetPanel].hide) and (HoN_Main.menuPanels.table[targetPanel].show)  then
					...
					if (forceOpen) then
						if (not isVisible) then
							...
							HideMainCornerAds()
							HoN_Main.menuPanels.table[targetPanel].show(panelWidget, HoN_Main.menuPanels.table[targetPanel].offset)
						...
						trackedUserFocus:logFocus('mainMenuPanel', targetPanel)
					elseif (forceClose) then
						if (isVisible) then
							HoN_Main.menuPanels.currentPanel = ''
							--Set('_mainmenu_currentpanel', '')
							print('should do hidecmd here 01\n')
						...
						trackedUserFocus:logFocus('mainMenuPanel', nil)
					else
						if (isVisible) then
							ShowMainCornerAds()
							HoN_Main.menuPanels.currentPanel = ''
							--Set('_mainmenu_currentpanel', '')
							print('should do hidecmd here 02\n')
							if (HoN_Main.menuPanels.table[targetPanel].hidecmd) then
								panelWidget:UICmd(""..(HoN_Main.menuPanels.table[targetPanel].hidecmd).."")
							end
							HoN_Main.menuPanels.table[targetPanel].hide(panelWidget)
...
local function GamePhase(self, gamePhase) -- 0 main, 1 lobby, 3 hero picker, 6 ingame
	trackedUserFocus:logFocus('gamePhase', AtoN(gamePhase))
	local gamePhase = tonumber(gamePhase)

	if gamePhase == lastGamePhase then
		return
	end

	lastGamePhase = gamePhase
	
	if gamePhase == 5 or gamePhase == 6 then
		if GetCvarBool('ui_game_specInterfaceLoaded') then
			Set('ui_match_is_spectating', 'true', 'bool')
		else
			Set('ui_match_is_spectating', 'false', 'bool')
		end
	end
	...
	if (gamePhase == 0) then
		...
		if ((WaitingToShowStats() or GetCvarBool('ui_match_stats_waitingToShow')) and not GetCvarBool('ui_match_is_spectating') and (GetShowStatsMatchID() ~= 4294967295)) then
		...
		if UIManager.GetInterface('game') then
			for i=1,3,1 do
...
local function TMMReadyStatus(self)
	MainMenuButtonsHandler()
end
interface:RegisterWatch('TMMReadyStatus', TMMReadyStatus)
...
function HoN_Main:ListModdedFiles()
	trackedUserFocus:logFocus('viewingModdedFiles', true)
...
function PrivacyPolicyManager:OnMainLoginPasswordEvent()
	if GetAgreePrivacyPolicy() or not GetCvarBool('cg_enablePrivacyPolicy') then
		interface:UICmd('Set(login_temp_password, MD5(main_login_password)); Login(); SaveLoginInfo(); If(!login_rememberPassword, EraseInputLine());')
	else
		PrivacyPolicyManager:ShowPrivacyPolicy(true)
	end						
end

function PrivacyPolicyManager:ShowPrivacyPolicy(showButton)
	GetWidget('new_privacy_policy_popup'):FadeIn(150)
	GetWidget('new_privacy_policy_popup_confirm'):SetVisible(showButton)

	local totalHeight = GetWidget('new_privacy_policy_popup_content'):GetHeight()
	local maxHeight = GetWidget('new_privacy_policy_popup_content_root'):GetHeight()

	local scrollWidget = GetWidget('new_privacy_policy_popup_scrollbar')

	if totalHeight > maxHeight then
		scrollWidget:SetVisible(true)

		local scrollMaxValue = totalHeight - maxHeight
		scrollWidget:SetMaxValue(scrollMaxValue)
		scrollWidget:SetValue(0)

		PrivacyPolicyManager:UpdatePrivacyPolicy()
	else
		scrollWidget:SetVisible(false)
	end
end

function PrivacyPolicyManager:UpdatePrivacyPolicy()
	local startIndex = tonumber(GetWidget('new_privacy_policy_popup_scrollbar'):GetValue())
	local maxValue = tonumber(GetWidget('new_privacy_policy_popup_scrollbar'):UICmd("GetScrollbarMaxValue()"))
	if startIndex < 0 then startIndex = 0 end
	if startIndex > maxValue then startIndex = maxValue end

	GetWidget('new_privacy_policy_popup_content'):SetY('-'..tostring(startIndex))
end

function PrivacyPolicyManager:CancelLogin()
	interface:UICmd('CancelLogin();')
end