...
local tabs = {'live', 'sea_matches', 'naeu_matches', 'bet_history'}
...
local function GetRankLevelByMMR(mmr)
	local config = GetCvarString('cc_mmrConfig')
...
local function GetMatchNames(matchData)
	...
	if matchData ~= nil then
		...
		elseif ruleType == 5 then
			...
			names.name = Translate('watch_system_campaign_match_name', 'rank', Translate('player_compaign_level_name_'..level))
