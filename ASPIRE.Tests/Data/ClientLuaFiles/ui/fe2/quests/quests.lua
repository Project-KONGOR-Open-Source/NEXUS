...
local rewardIconSilver		= '/ui/fe2/quests/textures/silver_icon_large.tga'
...
local function questsRegisterReward(object, index, isAvailableInit, isMetInit, isOpenedInit)
	...
	local function updateIcon()
		...
		if bagID ~= -1 then
			local suffixBig	= ''
			if index == 14 then
				bagTexture = '/ui/fe2/quests/textures/reward_chest_big'
			elseif rewardType == REWARD_TYPE_PRODUCT then
				bagTexture = '/ui/fe2/quests/textures/reward_chest'..suffixBig
			elseif rewardType == REWARD_TYPE_SILVER or rewardType == REWARD_TYPE_GOLD then
				bagTexture = '/ui/fe2/quests/textures/reward_bag'..suffixBig
			else
				bagTexture = '/ui/fe2/quests/textures/reward_bag'
...
local function questsRegister(object)
	...
	function rewardsUpdateScrollButtons()		-- can't disable due to needing proper onmouselup
		-- local nextTexture	= "/ui/fe2/quests/textures/scroll_button_up.tga"
		-- local prevTexture	= "/ui/fe2/quests/textures/scroll_button_up.tga"

		nextEnabled = true
		prevEnabled = true

		if rewardsScrollTargPos <= 0 then
			-- prevTexture = "/ui/fe2/quests/textures/scroll_button_disabled.tga"
		...
		if rewardsScrollTargPos >= rewardsScrollMax then
			-- nextTexture = "/ui/fe2/quests/textures/scroll_button_disabled.tga"
			nextEnabled = false
		...
		local textureDisabled		= '/ui/fe2/quests/textures/scroll_button_disabled.tga'
		local textureDown			= '/ui/fe2/quests/textures/scroll_button_down.tga'
		local textureUp				= '/ui/fe2/quests/textures/scroll_button_up.tga'
	...
	if HoN_Region.regionTable[HoN_Region.activeRegion].questSystem then
		...
		--set chat sizes etc if quests are on
		GetWidget('questsCommunicatorContainer'):SetHeight('-4.75h')
		GetWidget('communicatorTabsHasQuests'):SetHeight('3.5h')
		repositionRewards()
