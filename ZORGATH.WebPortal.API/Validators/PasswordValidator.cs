...
public class PasswordValidator : AbstractValidator<string>
{
    ...
    public PasswordValidator()
    {
        ...
        RuleFor(password => password).MinimumLength(MinimumPasswordLength).WithMessage($"Password Must To Be At Least {MinimumPasswordLength} Characters Long");

        RuleFor(password => password).Must(password => MeetsMinimumAlphabeticCharactersCountRequirement(password, MinimumAlphabeticCharactersCount))
            .WithMessage($"Password Must Contain At Least {MinimumAlphabeticCharactersCount} Alphabetic Characters");

        RuleFor(password => password).Must(password => MeetsMinimumNumericCharactersCountRequirement(password, MinimumNumericCharactersCount))
            .WithMessage($"Password Must Contain At Least {MinimumNumericCharactersCount} Numeric Characters");

        RuleFor(password => password).Must(password => MeetsMinimumSpecialCharactersCountRequirement(password, MinimumSpecialCharactersCount))
